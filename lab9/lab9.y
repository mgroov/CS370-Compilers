...
vardec:typespec varlist';' {$$ =$2;    
			     ...
			     while(p!=NULL){
			       ...
			       p=p->s1;
                           ...
                           ;
varlist:ID {
  $$ = ASTCreateNode(vardec);                                                                                                                                                                                                                   $$->Name = $1;
  $$->size =1;                                                                                                                                                                                                                                 if(Search($1,level,0) != NULL){                                                                                                                                                                                                                yyerror("duplicate declaration \n");
    yyerror($1);
    exit(1);                                                                                                                                                                                                                                   }                                                                                                                                                                                                                                            else{
...
param:typespec ID   {  
    ...
    if(Search($2,level+1,0) != NULL){
      ...
      $$->symbol=Insert($2,$1,0,level+1,1,offset,NULL);                                                                                                                                                                                            /*Insert(char *name, enum OPERATOR Type, int isafunc, int  level, int mysize, int offse,
																														     Tnode * fparms ); */
      offset+=1;
      Display();
